"""Standalone verification tests for Ticket 6: Prompt layering and context assembly."""
import json
from pathlib import Path
from datetime import datetime
from enum import Enum

# Mock User models for standalone testing
class UserRole(Enum):
    USER = "user"
    ADMIN = "admin"
...
class User:
    def __init__(self, id, email, name, role, approval_status):
        self.id = id
        self.email = email
        self.name = name
        self.role = role
...
# Import the actual services we're testing
import sys
...
def test_layer_ordering():
    ...
    # Generate prompt multiple times WITHOUT dynamic context (which has timestamp)
    prompts = [
        assemble_system_prompt(user, available_tools=["tool1", "tool2"], additional_context=None)
        for _ in range(5)
    ]

    # Verify all prompts are identical (check content, not object identity)
    for i in range(1, len(prompts)):
        assert len(prompts[i]) == len(prompts[0]), f"Prompt {i} has different layer count"
        for j, (layer1, layer2) in enumerate(zip(prompts[i], prompts[0])):
            assert layer1 == layer2, f"Prompt {i} layer {j} differs from prompt 0"
